...
class CompressionGUI:
    ...
    def setup_ui(self):
        ...
        # 左侧面板 - 基本设置
        left_panel = ttk.LabelFrame(main_frame, text="基本设置", padding="5")
        left_panel.grid(row=0, column=0, padx=5, pady=5, sticky=(tk.N, tk.S, tk.E, tk.W))
        
        # 文件选择区域
        ttk.Label(left_panel, text="输入路径:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(left_panel, textvariable=self.input_path).grid(row=0, column=1, sticky=(tk.E, tk.W), padx=5)
        ttk.Button(left_panel, text="浏览", command=self.browse_input).grid(row=0, column=2, padx=5)
        
        ttk.Label(left_panel, text="输出路径:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(left_panel, textvariable=self.output_path).grid(row=1, column=1, sticky=(tk.E, tk.W), padx=5)
        ttk.Button(left_panel, text="浏览", command=self.browse_output).grid(row=1, column=2, padx=5)
        
        # 基本压缩选项
        ttk.Label(left_panel, text="压缩级别:").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        levels = ["STORE", "FASTEST", "FAST", "NORMAL", "MAXIMUM", "ULTRA"]
        ttk.Combobox(left_panel, textvariable=self.compression_level, values=levels, state="readonly").grid(
            row=2, column=1, columnspan=2, sticky=(tk.E, tk.W), padx=5)
        
        ttk.Label(left_panel, text="线程数:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=2)
        ttk.Entry(left_panel, textvariable=self.thread_count).grid(row=3, column=1, columnspan=2, sticky=(tk.E, tk.W), padx=5)
        
        # 在左侧面板添加密码输入框
        ttk.Label(left_panel, text="加密密码:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=2)
        password_entry = ttk.Entry(left_panel, textvariable=self.password, show="*")  # 使用*隐藏密码
        ...
        ttk.Checkbutton(left_panel, text="显示密码", 
                        variable=self.show_password, 
                        command=toggle_password).grid(row=5, column=1, sticky=tk.W, padx=5)
        
        # 右侧面板 - 压缩详情
        right_panel = ttk.LabelFrame(main_frame, text="压缩详情", padding="5")
        right_panel.grid(row=0, column=1, padx=5, pady=5, sticky=(tk.N, tk.S, tk.E, tk.W))
        
        # 压缩级别说明
        level_desc = tk.Text(right_panel, height=8, width=40, wrap=tk.WORD)
        level_desc.grid(row=0, column=0, padx=5, pady=5, sticky=(tk.N, tk.S, tk.E, tk.W))
        level_desc.insert(tk.END, "选择压缩级别查看详细说明...")
        level_desc.configure(state='disabled')
        
        # 系统监控区域
        monitor_frame = ttk.LabelFrame(main_frame, text="系统监控", padding="5")
        monitor_frame.grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.E, tk.W))
        
        ttk.Label(monitor_frame, textvariable=self.cpu_usage).grid(row=0, column=0, padx=5)
        ttk.Label(monitor_frame, textvariable=self.memory_usage).grid(row=0, column=1, padx=5)
        ttk.Label(monitor_frame, textvariable=self.estimated_time).grid(row=0, column=2, padx=5)
        
        # 进度条
        self.progress = ttk.Progressbar(main_frame, mode='determinate', variable=self.progress_var)
        self.progress.grid(row=2, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.E, tk.W))
        
        # 状态日志
        log_frame = ttk.LabelFrame(main_frame, text="状态日志", padding="5")
        log_frame.grid(row=3, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.N, tk.S, tk.E, tk.W))
        ...
        # 配置网格权重，使组件能够随窗口调整大小
        main_frame.grid_rowconfigure(3, weight=1)  # 状态日志区域可扩展
    ...
    def browse_input(self):
        path = filedialog.askopenfilename(title="选择要压���文件") or \
